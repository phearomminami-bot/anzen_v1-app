...
import { STUDENTS, INSTRUCTORS, VEHICLES } from './data.js';

export const Photo = ({ tag, w, h, r = 6, style }) => (
  <div className="ph" style={{ width: w, height: h, borderRadius: r, ...style }}>
    {tag}
  </div>
);

export const Avatar = ({ tag, size = 36, ring }) => (
...
export const Card = ({ children, style, pad = 16, label }) => (
  <div style={{
...
export const Stat = ({ label, value, sub, delta }) => (
  <div>
    <div style={{
      font: '500 10px/1 "JetBrains Mono", monospace',
      letterSpacing: '.08em', textTransform: 'uppercase',
...
export const Btn = ({ children, kind = 'ghost', size = 'md', icon, onClick, style }) => {
  const sizes = {
    sm: { h: 28, px: 10, fs: 12 },
    md: { h: 34, px: 14, fs: 13 },
    lg: { h: 42, px: 18, fs: 14 },
...
export const SectionTitle = ({ km, en, action }) => (
...
export const Spark = ({ data, w = 100, h = 28, color = 'var(--accent)' }) => {
...
export const instById    = (id) => INSTRUCTORS.find(i => i.id === id);
export const vehById     = (id) => VEHICLES.find(v => v.id === id);
