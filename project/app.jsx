...
const DETAILS = {
  ...
  exam:         { Component: ExamDetail,        titleFn: (d) => { const k = window.__SCHED_KIND ? window.__SCHED_KIND(d && d.kind) : {km:'ប្រឡង',en:'Exam'}; return {km:k.km, en:k.en+' detail'}; } },
...
function App() {
  ...
  const renderLayout = () => {
    ...
    if (bp.mobile) {
      ...
      const tabName = curNav
        ? (lang === 'km' ? curNav.km : lang === 'jp' ? (curNav.jp || curNav.en) : curNav.en)
        : (window.__schoolSettings?.name || 'Anzen');
      // Dashboard shows the school name + today's date; other tabs show the tab title.
